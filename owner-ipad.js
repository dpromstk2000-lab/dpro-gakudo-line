(() => {
  ...
  async function ipadAlertAction(id,action){if(action==='resolve'&&!confirm('この警告を解決済みにしますか？'))return;await safeRun(async()=>{await A.ownerApi(`/admin/notifications/alerts/${id}/action`,{method:'POST',body:{action,note:''}});await loadAll()},'警告を更新しました。')}
